...
if (isset($_POST['schedule'])) {
  ...
  $name = $_POST['name'];
   ...
   $sql = mysqli_query($conn, "SELECT email FROM faculty WHERE name = '$name' ");
   $res=mysqli_fetch_assoc($sql);
   $email = $res['email'];

  mysqli_query($conn, "INSERT INTO invigallot (`name`, `room`, `email`) VALUES('$name', '$room', '$email')");
            ...
            <select name="name" required>
              <option></option>
               <?php
                ...
                $branchq=mysqli_query($conn, "SELECT * from faculty WHERE email NOT IN(select email from invigallot)");
            ...
            <select name="room" required>
              <option></option>
               <?php
                ...
                $branchq1=mysqli_query($conn, "SELECT distinct classroom from trisub1 WHERE classroom NOT IN(select room from invigallot)");
                while($row1 =mysqli_fetch_assoc($branchq1)) {
                  # code...
                  $classroom1 = $row1['classroom'];
                   ?> 
                   <option value="<?php echo($classroom1);?>">
                    <?php echo($classroom1);?>
                  </option>
