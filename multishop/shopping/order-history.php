...
if (isset($_POST['removeproduct'])) {
	# code...
	$id = $_POST['cancelproduct'];
	$sql = $con->prepare("DELETE FROM `orders` WHERE  productId = ? AND userId = ?");
    $sql->bind_param("ii", $id, $userId);
    $sql ->execute();
    if($sql) echo "<script>alert('Order cancelled');</script>";
		...
		<table class="table table-bordered">
			<thead>
				<tr>
					...
					<th class="cart-total last-item" colspan="2">Action</th>
				</tr>
			</thead><!-- /thead -->
			
			<tbody>

<?php $query=mysqli_query($con,"select products.id as id,products.productImage1 as pimg1,products.productName as pname,products.id as proid,orders.productId as opid,orders.quantity as qty,products.productPrice as pprice,products.shippingCharge as shippingcharge,orders.paymentMethod as paym,orders.orderDate as odate,orders.id as orderid from orders join products on orders.productId=products.id where orders.userId='".$_SESSION['id']."' and orders.paymentMethod is not null");
$cnt=1;
 ...
 <a href="javascript:void(0);" onClick="popUpWindow('track-order.php?oid=<?php echo htmlentities($row['orderid']);?>');" title="Track order">
					...
					<td class="cart-product-sub-total">
						<form action="" method="post">
							<input type="hidden" name="cancelproduct" value="<?php echo $row['id'];?>">
							<input type="submit" name="removeproduct" value="cancel Order">
						</form>
					</td>
				</tr>
<?php $cnt=$cnt+1; ?>
...
?>
	...
	<h3 class="section-title">Also you know</h3>
	<div class="owl-carousel home-owl-carousel upsell-product custom-carousel owl-theme outer-top-xs">
	   
		<?php 
$qry=mysqli_query($con,"select * from relateditem where subCategory='$subcid' and category='$cid' and relatedIteam = $pid ");
while($rw=mysqli_fetch_array($qry))
{
	...
	<div class="product">		
		...
		<div class="product-info text-left">
			<h3 class="name"><?php echo htmlentities($rw['productName']);?></h3>
